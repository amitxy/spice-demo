	...
	async setConstitution(content: string): Promise<void> {
		...
		try {
			...
			const rootId = rootMessage
				? rootMessage.id
				: await DatabaseService.createRootMessage(activeConv.id);
			const existingSystem = allMessages.find(
				(m) => m.role === MessageRole.SYSTEM && m.parent === rootId
			);
			if (existingSystem) {
				await DatabaseService.updateMessage(existingSystem.id, { content: prompt });
				const idx = conversationsStore.findMessageIndex(existingSystem.id);
				if (idx !== -1) conversationsStore.updateMessageAtIndex(idx, { content: prompt });
				return;
			}
			const firstActive = conversationsStore.activeMessages.find((m) => m.parent === rootId);
			const systemMessage = await DatabaseService.createSystemMessage(activeConv.id, prompt, rootId);
			if (firstActive) {
				await DatabaseService.updateMessage(firstActive.id, { parent: systemMessage.id });
				await DatabaseService.updateMessage(systemMessage.id, { children: [firstActive.id] });
				const updatedRootChildren = rootMessage
					? rootMessage.children.filter((id: string) => id !== firstActive.id)
					: [];
				await DatabaseService.updateMessage(rootId, {
					children: [
						...updatedRootChildren.filter((id: string) => id !== systemMessage.id),
				...
				const firstMsgIndex = conversationsStore.findMessageIndex(firstActive.id);
				if (firstMsgIndex !== -1)
