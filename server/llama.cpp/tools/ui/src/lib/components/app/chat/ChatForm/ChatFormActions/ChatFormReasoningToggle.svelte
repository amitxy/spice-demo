<script lang="ts">
	import { Lightbulb, Check, Info } from '@lucide/svelte';
	...
	let tooltipText = $derived(`${currentEffort} Reasoning`);
	...
	function isSelected(item: ReasoningEffortLevel): boolean {
		return currentEffort === item.value;
	}
		...
		<Tooltip.Root>
			<Tooltip.Trigger>
				<DropdownMenu.Trigger
					class="flex h-6 w-6 cursor-pointer items-center justify-center rounded-full p-0 bg-amber-400/10 hover:bg-amber-400/20 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
